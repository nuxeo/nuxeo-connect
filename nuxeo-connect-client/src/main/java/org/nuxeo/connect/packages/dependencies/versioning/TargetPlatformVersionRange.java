 ...
 * under the License.
 * 
 * @origin https://github.com/apache/maven/blob/master/maven-artifact/src/main/java/org/apache/maven/artifact/versioning/VersionRange.java
 */
...
public class TargetPlatformVersionRange {
    private static final Map<String, TargetPlatformVersionRange> CACHE_SPEC = Collections.synchronizedMap(
            new WeakHashMap<>());

    private static final Map<String, TargetPlatformVersionRange> CACHE_VERSION = Collections.synchronizedMap(
            new WeakHashMap<>());

    private final TargetPlatformVersion recommendedVersion;

    private final List<Restriction> restrictions;

    private TargetPlatformVersionRange(TargetPlatformVersion recommendedVersion, List<Restriction> restrictions) {
        this.recommendedVersion = recommendedVersion;
        this.restrictions = restrictions;
    }

    public TargetPlatformVersion getRecommendedVersion() {
     ...
     * @return a new {@link TargetPlatformVersionRange} object that represents the spec
     * @throws InvalidVersionSpecificationException
     */
    public static TargetPlatformVersionRange createFromVersionSpec(String spec)
            throws InvalidVersionSpecificationException {
        if (spec == null) {
            return null;
        }

        TargetPlatformVersionRange cached = CACHE_SPEC.get(spec);
        if (cached != null) {
            return cached;
        }
        ...
        TargetPlatformVersion upperBound = null;
        TargetPlatformVersion lowerBound = null;
        ...
        if (process.length() > 0) {
            if (restrictions.size() > 0) {
                ...
                version = new DefaultTargetPlatformVersion(process);
                restrictions.add(Restriction.EVERYTHING);
            }
        }

        cached = new TargetPlatformVersionRange(version, restrictions);
    ...
    private static Restriction parseRestriction(String spec) throws InvalidVersionSpecificationException {
        ...
        if (index < 0) {
            ...
            TargetPlatformVersion version = new DefaultTargetPlatformVersion(process);
            ...
            TargetPlatformVersion lowerVersion = null;
            if (lowerBound.length() > 0) {
                lowerVersion = new DefaultTargetPlatformVersion(lowerBound);
            }
            TargetPlatformVersion upperVersion = null;
            if (upperBound.length() > 0) {
                upperVersion = new DefaultTargetPlatformVersion(upperBound);
            }
    ...
    public static TargetPlatformVersionRange createFromVersion(String version) {
        TargetPlatformVersionRange cached = CACHE_VERSION.get(version);
        if (cached == null) {
            List<Restriction> restrictions = Collections.emptyList();
            cached = new TargetPlatformVersionRange(new DefaultTargetPlatformVersion(version), restrictions);
    ...
    /**
     * Creates and returns a new <code>TargetPlatformVersionRange</code> that is a restriction of this version range and
     ...
     * </p>
     *
     * @param restriction the <code>TargetPlatformVersionRange</code> that will be used to restrict this version range.
     * @return the <code>TargetPlatformVersionRange</code> that is a restriction of this version range and the specified
     *         version range.
     *         <p>
     ...
     *         </p>
     * @throws NullPointerException if the specified <code>TargetPlatformVersionRange</code> is <code>null</code>.
     */
    public TargetPlatformVersionRange restrict(TargetPlatformVersionRange restriction) {
        ...
        // Either the original or the specified version ranges have no restrictions
            ...
            // Use the recommended version from the specified TargetPlatformVersionRange since there is no
        ...
        return new TargetPlatformVersionRange(version, restrictions);
    }
        ...
        while (!done) {
            if (res1.getLowerBound() == null || res2.getUpperBound() == null
                ...
                if (res1.getUpperBound() == null || res2.getLowerBound() == null
                    ...
                    TargetPlatformVersion lower;
                    TargetPlatformVersion upper;
    ...
    public TargetPlatformVersion matchVersion(List<TargetPlatformVersion> versions) {
        // TODO could be more efficient by sorting the list and then moving along the restrictions in order?

        TargetPlatformVersion matched = null;
        for (TargetPlatformVersion version : versions) {
    ...
    public boolean equals(Object obj) {
        ...
        if (!(obj instanceof TargetPlatformVersionRange)) {
            return false;
        }
        TargetPlatformVersionRange other = (TargetPlatformVersionRange) obj;
