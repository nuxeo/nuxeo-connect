...
/**
 * Test feature introduced by NXBT-654:
 * "Exclude SNAPSHOT packages from resolution if not relevant". SNAPSHOT must be
 * excluded by default, included if allowed by option, included if explicitly
 * needed by the request.
 *
 * Test (install, upgrade and downgrade) by (name or ID) with (SNAPSHOT allowed
 * or not) and package already installed in the (required, greater, lower
...
public class TestSNAPSHOT extends AbstractPackageManagerTestCase {
    ...
    public void testBWithoutSNAPSHOT() throws Exception {
        ...
        assertEquals(new Version("1.0.1-SNAPSHOT"),
                depResolution.getLocalPackagesToUpgrade().get("B"));
    ...
    public void testAWithSNAPSHOT() throws Exception {
        List<String> installs = new ArrayList<String>();
        installs.add("A");
        DependencyResolution depResolution = pm.resolveDependencies(installs,
                null, null, null, true);
        log.info(depResolution.toString());
        assertTrue(depResolution.isValidated());
        assertEquals(1, depResolution.getLocalPackagesToInstall().size());
